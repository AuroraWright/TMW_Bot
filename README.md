...
Maintains a one-way structural mirror from the configured main guild to its sub-server destinations. It copies regular role properties and order, channel/category layout and permission overwrites, supported guild settings, member roles, and as many static and animated emojis as the destination's current slots permit. Member roles are translated through persisted main-to-sub-server role mappings; startup and periodic reconciliation also backfill members already in each sub-server, with a REST lookup when the main-guild member cache is incomplete. The destination server name can remain locally managed by disabling `mirror_guild_name` in `config/sub_server_settings.yml`. Persisted ID mappings make startup reconciliation resumable and prevent duplicates. Periodic reconciliation fills newly available emoji slots later.
